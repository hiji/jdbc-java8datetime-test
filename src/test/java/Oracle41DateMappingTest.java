...
public class Oracle41DateMappingTest {
...
//        java.sql.Timestamp ts = new Timestamp(of.getYear(), of.getMonthValue(), of.getDayOfMonth(), of.getHour(), of.getMinute(), of.getSecond(), of.getNano());
    ...
    @Test
    public void 時差を別で更新する() throws Exception {
        PreparedStatement stmt = conn.prepareStatement("UPDATE JDBC_TEST SET ts=?, ts_offset=? WHERE id='hoge'");

        OffsetDateTime offsetDateTime = OffsetDateTime.of(2016, 4, 26, 10, 11, 12, 123456789, ZoneOffset.of("+08:00"));
    ...
    @Test
    public void ZonedDateTimeで更新する() throws Exception {
        PreparedStatement stmt = conn.prepareStatement("UPDATE JDBC_TEST SET ts=?, ts_zone=? WHERE id='hoge'");

        ZonedDateTime zonedDateTime = ZonedDateTime.of(2016, 4, 26, 10, 11, 12, 123456789, ZoneId.of("Asia/Tokyo"));
    ...
    @Test
    public void ZonedDateTimeを取得する() throws Exception {
        ...
        Instant date = new Date(timestamp.getTime()).toInstant();
        ZonedDateTime result = ZonedDateTime.ofInstant(date, ZoneId.of(offset));

        assertThat(result, is(ZonedDateTime.of(2016, 4, 26, 10, 11, 12, 123000000, ZoneId.of("Asia/Tokyo"))));
    ...
    @Test
    public void ZonedDateTimeの比較() throws Exception {


        ZonedDateTime asia = ZonedDateTime.of(2016, 4, 26, 10, 11, 12, 123000000, ZoneId.of("Asia/Tokyo"));
        ZonedDateTime utc = ZonedDateTime.of(2016, 4, 26, 1, 11, 12, 123000000, ZoneId.of("UTC"));
        assertThat(Date.from(asia.toInstant()), is(Date.from(utc.toInstant())));
    ...
    @Test
    public void 時差を別で取得する() throws Exception {
        PreparedStatement statement = conn.prepareStatement("SELECT * FROM JDBC_TEST WHERE id='hoge'");
        ResultSet resultSet = statement.executeQuery();
        resultSet.next();
        ...
        OffsetDateTime result = OffsetDateTime.of(localDateTime, ZoneOffset.of(offset));
...
//    }
    ...
    public void DATE型をDateとしてgetObjectで取得する() throws Exception {
        Date actual = resultSet.getObject("dt", Date.class);
    ...
    public void TIMESTAMP型をDateとしてgetObjectで取得する() throws Exception {
        Date actual = resultSet.getObject("ts", Date.class);
